...
export const admin_manage_subscriptionRouter = Router();
...
admin_manage_subscriptionRouter.post('/', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[admin_manage_subscription] Error:', error);
    res.status(500).json({ 
      error: error instanceof Error ? error.message : 'Internal server error',
      route: 'admin-manage-subscription'
    });
  }
});
