...
export const github_sync_webhookRouter = Router();
...
github_sync_webhookRouter.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const supabaseAdmin = createClient(
        ...
        const diffCleanResponse = await fetch(
          `${process.env.SUPABASE_URL}/functions/v1/diff-clean`,
          {
            method: 'POST',
            headers: {
    ...
    console.error('[github_sync_webhook] Error:', error);
    res.status(500).json({ 
      error: error instanceof Error ? error.message : 'Internal server error',
      route: 'github-sync-webhook'
    });
  }
});
