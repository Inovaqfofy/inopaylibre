...
export const inspect_github_repoRouter = Router();
...
inspect_github_repoRouter.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const supabaseUrl = process.env.SUPABASE_URL;
        ...
        const { github_repo_url, branch } = req.body;
    ...
    console.error('[inspect_github_repo] Error:', error);
    res.status(500).json({ 
      error: error instanceof Error ? error.message : 'Internal server error',
      route: 'inspect-github-repo'
    });
  }
});
