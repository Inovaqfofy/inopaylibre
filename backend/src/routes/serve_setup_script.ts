...
export const serve_setup_scriptRouter = Router();
...
serve_setup_scriptRouter.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const url = new URL(req.url);
        ...
        if (!setupId) {
          return res.status(400).send('Missing setup ID');
        }
        ...
        if (error || !server) {
          return res.status(404).send('Invalid setup ID');
        }
    ...
    console.error('[serve_setup_script] Error:', error);
    res.status(500).json({ 
      error: error instanceof Error ? error.message : 'Internal server error',
      route: 'serve-setup-script'
    });
  }
});
