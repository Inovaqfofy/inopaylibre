...
const SYNTAX_ERROR_PATTERNS: Array<{
  ...
  { pattern: /\s*;?/g, message: "détecté", severity: "warning" }];
...
function autoFixFiles(files: Record<string, string>): AutoFixResult {
  ...
  for (const [filePath, content] of Object.entries(files)) {
    ...
    // 2. Remove statements
    const Pattern = /\bdebugger\s*;?\n?/g;
    const Matches = fixed.match(Pattern);
    if (Matches) {
      fixed = fixed.replace(Pattern, '');
      patternCounts[''] = (patternCounts[''] || 0) + Matches.length;
      totalReplacements += Matches.length;
    }
    
    // 3. Remove empty imports: const emptyImportPattern = /import\s*{\s*}\s*from\s*['"][^'"]+['"];?\n?/g;
